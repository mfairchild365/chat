...
class Login extends User implements PostHandlerInterface
{
    public function handlePost($get, $post, $files)
    {
        ...
        if (!isset($post['username']) || empty($post['username'])) {
            throw new \Chat\Exception("You need to specify a username", 400);
        }
        ...
        if (!$found) {
            throw new \Chat\Exception("We were not able to find that user.  Are you sure you have an account?", 400);
        }

        if (!password_verify($post['password'], $user->password)) {
            throw new \Chat\Exception("Sorry, we could not find that account.", 400);
        }
